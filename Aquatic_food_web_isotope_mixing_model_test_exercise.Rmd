...
# (1pt) Check that the conversion to matrix format worked by asking R for the class of the source.means3 object on the line below. Remember, the command for this is class(object.name).
...
# We will subset the source.means object to get just the rows and column we need, dropping those for which we have only one observation
...
# (2pts) What is the class of the source.sds3 object? Why do we need it to be this class? Remember that you can find the class using the command class(objectname)
...
(2pts) Create the source means object. Remember that you will need to do this in three steps and end up with numeric values in a matrix. Each of these steps needs to be in a code chunk.
